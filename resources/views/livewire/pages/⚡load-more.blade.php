...
<div class="p-8">
    <div class="max-w-5xl mx-auto">
        ...
        <!-- Info Card -->
        <div class="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800 p-6 mb-8">
            ...
            <p class="text-sm text-emerald-800 dark:text-emerald-200 mb-3">
                The "Load More" button uses <code class="px-1.5 py-0.5 bg-emerald-200 dark:bg-emerald-800 rounded">wire:island</code> to target the named island. 
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6 mb-6">
            <div class="mb-6">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400">Page {{ $page }} • Showing {{ $perPage }} per page</p>
            </div>

            @island(name: 'user-grid')
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($users as $user)
                        <div class="bg-gradient-to-br from-zinc-50 to-zinc-100 dark:from-zinc-700 dark:to-zinc-800 rounded-lg p-6 border border-zinc-200 dark:border-zinc-600 hover:shadow-lg transition-shadow">
                            <div class="flex flex-col items-center text-center">
                                <div class="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center text-white font-bold text-2xl mb-3">
                                ...
                                <span class="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-xs font-medium rounded-full">
                                    ID: {{ $user->id }}
                                </span>
                            </div>
                        </div>
            ...
            @else
                <div class="mt-8 text-center p-4 bg-zinc-50 dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <pre class="bg-zinc-900 text-zinc-100 p-4 rounded-lg overflow-x-auto text-xs"><code>@island(name: 'user-grid')
    <div class="grid">
        @foreach($users as $user)
            <div>{{ $user->name }}</div>
        @endforeach
