...
<div class="p-8">
    <div class="max-w-4xl mx-auto">
        ...
        <!-- Chat Container -->
        <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden flex flex-col" style="height: 600px;">
            <!-- Chat Header -->
            <div class="p-4 border-b border-zinc-200 dark:border-zinc-700 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20">
                    ...
                    @foreach($messages as $message)
                        <div class="chat-message">
                            <div class="flex gap-3">
                                <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                                    {{ strtoupper(substr($message->user->name, 0, 1)) }}
                                </div>
                                <div class="flex-1">
                                    <div class="flex items-baseline gap-2 mb-1">
                                        <span class="font-semibold text-sm text-zinc-900 dark:text-white">{{ $message->user->name }}</span>
                                        <span class="text-xs text-zinc-500 dark:text-zinc-400">{{ $message->created_at->format('H:i') }}</span>
                                    </div>
            ...
            <div class="p-4 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900">
                <form wire:submit="sendMessage" class="flex gap-3">
                    <input 
                        ...
                        class="flex-1 px-4 py-2 bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white"
                    >
                    <flux:button 
                        type="submit"
        ...
        <div class="mt-8 bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
            ...
            <pre class="bg-zinc-900 text-zinc-100 p-4 rounded-lg overflow-x-auto text-xs"><code>@island(name: 'messages')
    @foreach($messages as $message)
